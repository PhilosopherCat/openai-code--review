package plus.gaga.middleware.service;
...
public class ReviewApiService {
    ...
    private final Logger logger = LoggerFactory.getLogger(ReviewApiService.class);
     ...
     * 存储评审结果
     */
    private final Map<String, ReviewResponse> reviewCache = new ConcurrentHashMap<>();

    public ReviewApiService(GitCommand gitCommand, IOpenAI openAI, WeiXin weiXin) {
    ...
    public ReviewResponse submitReview(ReviewRequest request) {
        String taskId = "review-" + System.currentTimeMillis() + "-" + RandomStringUtils.randomNumeric(4);
        ...
        reviewCache.put(taskId, response);
        
        // 异步执行评审
        executeReviewAsync(taskId, request);
     ...
     * 查询评审结果
     */
    public ReviewResponse getReviewResult(String taskId) {
     ...
     */
    @Async
    public void executeReviewAsync(String taskId, ReviewRequest request) {
        try {
            String diffCode;
            
            if (request.getDiffContent() != null && !request.getDiffContent().isEmpty()) {
                // 直接使用传入的 diff 内容
                diffCode = request.getDiffContent();
            } else {
                // TODO: 实现从 GitHub 仓库拉取代码逻辑
                throw new RuntimeException("GitHub 仓库拉取功能暂未实现，请传入 diffContent 参数");
            }
            
            // 调用 AI 进行评审
            String reviewContent = codeReview(diffCode);
            
            // 记录评审结果
            String reviewUrl = recordCodeReview(reviewContent);
            
            // 解析评分
            Integer score = extractScore(reviewContent);
            
            // 更新结果
            ReviewResponse response = reviewCache.get(taskId);
            response.setStatus("success");
            response.setReviewContent(reviewContent);
            response.setReviewUrl(reviewUrl);
            response.setScore(score);
            
            // 发送微信通知
            if (Boolean.TRUE.equals(request.getEnableWeixinNotify())) {
                pushMessage(reviewUrl);
            }
            
        } catch (Exception e) {
            logger.error("Review failed for task: {}", taskId, e);
            ReviewResponse response = reviewCache.get(taskId);
            if (response != null) {
    ...
    private String recordCodeReview(String recommend) throws Exception {
        // 检查 GitHub 配置是否完整
        if (gitCommand != null && isGitConfigValid()) {
            return gitCommand.commitAndPush(recommend);
        }
        
        // 配置缺失，保存到本地文件
        logger.warn("GitHub config not found, saving review to local file");
        String dateFolderName = new SimpleDateFormat("yyyy-MM-dd").format(new Date());
        File dataDir = new File("openai-code-review-test/data/" + dateFolderName);
        if (!dataDir.exists()) {
            dataDir.mkdirs();
        }
        
        String fileName = "review-" + System.currentTimeMillis() + "-" + RandomStringUtils.randomNumeric(4) + ".md";
        File file = new File(dataDir, fileName);
        try (FileWriter writer = new FileWriter(file)) {
            writer.write(recommend);
        }
        
        logger.info("Review saved to local file: {}", file.getAbsolutePath());
        return "本地文件: " + file.getAbsolutePath();
    }
    
    private boolean isGitConfigValid() {
        try {
            String uri = gitCommand.getProject();
    ...
    private void pushMessage(String logUrl) throws Exception {
        ...
        plus.gaga.middleware.sdk.infrastructure.weixin.dto.TemplateMessageDTO.put(data, 
                plus.gaga.middleware.sdk.infrastructure.weixin.dto.TemplateMessageDTO.TemplateKey.REPO_NAME, 
                gitCommand.getProject());
        plus.gaga.middleware.sdk.infrastructure.weixin.dto.TemplateMessageDTO.put(data, 
                plus.gaga.middleware.sdk.infrastructure.weixin.dto.TemplateMessageDTO.TemplateKey.BRANCH_NAME, 
                gitCommand.getBranch());
        plus.gaga.middleware.sdk.infrastructure.weixin.dto.TemplateMessageDTO.put(data, 
                plus.gaga.middleware.sdk.infrastructure.weixin.dto.TemplateMessageDTO.TemplateKey.COMMIT_AUTHOR, 
                gitCommand.getAuthor());
        plus.gaga.middleware.sdk.infrastructure.weixin.dto.TemplateMessageDTO.put(data, 
                plus.gaga.middleware.sdk.infrastructure.weixin.dto.TemplateMessageDTO.TemplateKey.COMMIT_MESSAGE, 
     ...
     * 从评审内容中提取评分
     */
    private Integer extractScore(String reviewContent) {
        try {
            // 尝试从 "代码评分：XX" 中提取数字
            String[] lines = reviewContent.split("\n");
            for (String line : lines) {
                if (line.contains("代码评分")) {
            ...
            logger.warn("Failed to extract score from review content", e);
        }
        return null;
    }
}
